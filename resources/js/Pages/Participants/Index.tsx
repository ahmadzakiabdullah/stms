        ...
        >
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        {t('Athletes and teams registered in the system. Each participant can be registered for multiple tournaments.')}
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <Table>
            ...
            <Dialog open={!!deleteParticipant} onOpenChange={(isOpen) => !isOpen && setDeleteParticipant(null)}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {t('Are you sure you want to delete')} <strong>{deleteParticipant?.name}</strong>? {t('This will also remove all their tournament registrations.')} {t('This action cannot be undone.')}
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
            ...
            <div className="mt-6 text-xs text-muted-foreground">
                {t('M3: Participant module complete. Part of the Participant & Registration system.')}
            </div>
        </AuthenticatedLayout>
    );
}
...
function ParticipantFormDialog({ participant, sessions, onClose }: { participant: ParticipantRow | null; sessions: Session[]; onClose: () => void }) {
    ...
    const onSubmit = (e: React.FormEvent) => {
        ...
        try {
            ...
            setErrors({ logo: t('Unable to submit the form. Please refresh the page and try again.') });
