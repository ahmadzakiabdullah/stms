        ...
        >
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        {t("Click a sport to view and manage its categories. Categories define the competition divisions (e.g. Men's Singles, Women's Team).")}
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <Table>
                                    ...
                                    <TableRow className="cursor-pointer" onClick={() => toggleExpand(sport.id)}>
                                        ...
                                        <TableCell>
                                            <Badge variant="secondary" className="text-xs">
                                                {t('{{count}} categories', { count: sport.categories?.length ?? 0 })}
                                            </Badge>
                                        </TableCell>
                                        <TableCell>
                                            <span
                ...
                <DialogContent>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="grid gap-4 py-4">
                            ...
                            <div className="grid gap-2">
                                <Label htmlFor="icon">{t('Icon (Lucide name, optional)')}</Label>
                                <Input id="icon" {...register('icon')} placeholder={t('trophy or badminton')} />
                            </div>
                            <div className="grid gap-2">
            ...
            <Dialog open={!!deleteSport} onOpenChange={(isOpen) => !isOpen && setDeleteSport(null)}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {t('Are you sure you want to delete')} <strong>{deleteSport?.name}</strong>? {t('This action cannot be undone.')}
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        ...
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                {sports.find(s => s.id === catSportId)?.name
                                    ? t('Category for {{name}}', { name: sports.find(s => s.id === catSportId)!.name })
                            ...
                            <div className="border-t pt-4">
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <Input id="cat-max-male" type="number" min="0" {...catReg('max_male_athletes')} placeholder={t('e.g. 12')} />
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <Input id="cat-max-female" type="number" min="0" {...catReg('max_female_athletes')} placeholder={t('e.g. 12')} />
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <Input id="cat-max-officials" type="number" min="0" {...catReg('max_officials')} placeholder={t('e.g. 5')} />
            ...
            <Dialog open={!!deleteCat} onOpenChange={(isOpen) => !isOpen && setDeleteCat(null)}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {t('Are you sure you want to delete')} <strong>{deleteCat?.name}</strong>? {t('This action cannot be undone.')}
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setDeleteCat(null)}>{t('Cancel')}</Button>
                        <Button variant="destructive" onClick={handleCatDelete} disabled={catSubmitting}>{t('Yes, Delete')}</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            </>
            ...
            <div className="mt-6 text-xs text-muted-foreground">
                {t('M2: Sports with inline category management. Use the Categories menu for full category listing.')}
            </div>
        </AuthenticatedLayout>
    );
}
