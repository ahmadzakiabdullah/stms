                                                            ...
                                                            {!isFacultyRepresentative && ep.status === 'pending' && (
                                                                <>
                                                                    <button onClick={() => approveRegistration(ep.id)}
                                                                        className="inline-flex size-8 items-center justify-center rounded-md border border-emerald-200 text-emerald-600 transition hover:bg-emerald-600 hover:text-white" title="Approve" aria-label="Approve">
                                                                        <Check className="size-3.5" />
                                                                    </button>
                                                                    <button onClick={() => setRejectTarget({ epId: ep.id, participantName: participant.name, eventName: evt.name })}
                                                                        className="inline-flex size-8 items-center justify-center rounded-md border border-rose-200 text-rose-600 transition hover:bg-rose-600 hover:text-white" title="Reject" aria-label="Reject">
                                                                        <CircleX className="size-3.5" />
                                                                    </button>
                                                                </>
                                                            )}
                                                            <button onClick={() => setUnregTarget({ id: ep.id, participantName: participant.name, eventName: evt.name })}
                                                                className="inline-flex size-8 items-center justify-center rounded-md border border-input text-muted-foreground transition hover:bg-destructive hover:text-destructive-foreground" title="Unregister" aria-label="Unregister">
