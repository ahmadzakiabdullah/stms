...
function Content({ section, competitionName, t }: { section: Section; competitionName: string; t: (key: string) => string }) {
    ...
    if (section === 'faq') return <Accordion type="single" collapsible className="rounded-2xl border border-[var(--public-dark-border)] bg-white px-5 shadow-sm">{['Where can I find the latest schedule?', 'Where are official results published?', 'How can I contact the secretariat?'].map((question, index) => <AccordionItem key={question} value={`question-${index}`}><AccordionTrigger>{t(question)}</AccordionTrigger><AccordionContent>{t('Please refer to the official portal sections or contact the secretariat for the latest confirmed information.')}</AccordionContent></AccordionItem>)}</Accordion>;
