...
function MatchCard({ match, result = false, t }: { match: Match; result?: boolean; t: (key: string) => string }) {
    return <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
        <div className="flex items-center justify-between gap-3 text-xs font-semibold uppercase tracking-wider text-emerald-700"><span>{match.sport || t('Event')} · {match.event}</span><span className={match.status === 'in_progress' ? 'rounded-full bg-red-50 px-2 py-1 text-red-600' : 'text-slate-400'}>{match.status === 'in_progress' ? t('In progress') : result ? t('Completed') : formatDate(match.scheduled_at, true)}</span></div>
    ...
    return <><Head title={competition?.name || 'Sukan Antara Fakulti 2026'}><link rel="canonical" href="https://saf.utem.edu.my/portal/" /></Head>
        <div className="min-h-screen bg-[#f6f8f7] text-slate-950">
            ...
            <main id="utama">
                ...
                <section id="jadual" className="mx-auto max-w-7xl px-4 py-16 sm:px-6"><div className="mb-7 flex items-end justify-between"><div><p className="text-sm font-bold uppercase tracking-widest text-emerald-600">{t('Next')}</p><h2 className="mt-1 text-3xl font-black">{t('Match schedule')}</h2></div></div>{upcoming.length ? <div className="grid gap-4 lg:grid-cols-2">{upcoming.map(match=><MatchCard key={match.id} match={match} t={t}/>)}</div> : <Empty text={t('Schedule will be shown after publication.')} />}</section>
                <section id="keputusan" className="bg-white py-16"><div className="mx-auto max-w-7xl px-4 sm:px-6"><div className="mb-7 flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-widest text-emerald-600">{t('Latest')}</p><h2 className="mb-1 mt-1 text-3xl font-black">{t('Match results')}</h2><p className="text-xs text-slate-500">{t('Updated')} {formatDate(updated_at, true)} · {t('Auto-refreshes every 30 seconds')}</p></div><button type="button" onClick={() => refresh()} disabled={refreshing} className="flex items-center gap-2 rounded-full border border-emerald-600/30 bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-100 disabled:opacity-60"><RefreshCw className={`size-4 ${refreshing ? 'animate-spin' : ''}`} />{refreshing ? t('Refreshing') : t('Refresh')}</button></div>{results.length ? <div className="grid gap-4 lg:grid-cols-2">{results.map(match=><MatchCard key={match.id} match={match} result t={t}/>)}</div> : <Empty text={t('No official results recorded yet.')} />}</div></section>
