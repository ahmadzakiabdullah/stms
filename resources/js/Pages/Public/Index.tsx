...
type MedalRow = { rank: number; participant_name: string; logo_url?: string | null; gold: number; silver: number; bronze: number; total_medals: number };
type Props = {
    app_name: string;
...
export default function PublicIndex({ app_name, competition, stats, upcoming, results, medals, updated_at }: Props) {
    ...
    const refresh = () => {
        setRefreshing(true);
        router.reload({ only: ['stats', 'upcoming', 'results', 'medals', 'updated_at'], preserveScroll: true, onFinish: () => setRefreshing(false) });
    };
                ...
                <section className="relative isolate overflow-hidden bg-[var(--public-dark)] pb-16 pt-36 text-white sm:pb-20 sm:pt-44">
                    ...
                    <div className="mx-auto grid min-h-[520px] max-w-7xl items-center gap-12 px-4 sm:px-6 lg:grid-cols-[1.08fr_.92fr] xl:gap-20">
                        <div className="relative z-10 max-w-3xl">
                            ...
                            {competition?.start_date && <div className="mt-8 inline-flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white/65"><span className="flex size-8 items-center justify-center rounded-lg bg-white/10 text-[var(--public-highlight)]"><CalendarDays className="size-4" /></span><span><small className="block text-[9px] font-black uppercase tracking-[.16em] text-white/35">{t('Competition dates')}</small>{formatDate(competition.start_date, locale)}{competition.end_date && ` — ${formatDate(competition.end_date, locale)}`}</span></div>}
                        </div>
                        ...
                        <aside className="relative mx-auto w-full max-w-lg rounded-[1.75rem] border border-white/15 bg-white/[.07] p-3 shadow-2xl backdrop-blur-md">
                            <div className="rounded-[1.35rem] border border-white/10 bg-black/15 p-5 sm:p-7">
                                ...
                                <div className="mt-5 h-2 overflow-hidden rounded-full bg-white/10" role="progressbar" aria-valuenow={progress} aria-valuemin={0} aria-valuemax={100}><div className="h-full rounded-full bg-gradient-to-r from-[var(--public-primary)] via-[var(--public-accent)] to-[var(--public-highlight)]" style={{ width: `${progress}%` }} /></div>
                                ...
                                <div className="mt-5 border-t border-white/10 pt-5">
                                    <div className="flex items-center justify-between gap-3"><p className="text-[10px] font-black uppercase tracking-[.18em] text-white/40">{t('Next fixture')}</p><Clock3 className="size-4 text-[var(--public-accent)]" /></div>
                                    {upcoming[0] ? <div className="mt-4"><p className="truncate text-xs font-bold text-[var(--public-accent)]">{upcoming[0].sport} · {upcoming[0].event}</p><div className="mt-3 grid grid-cols-[1fr_auto_1fr] items-center gap-3"><TeamLabel team={upcoming[0].home} /><span className="rounded-lg bg-white/10 px-2.5 py-1.5 text-[10px] font-black">VS</span><TeamLabel team={upcoming[0].away} away /></div><p className="mt-3 flex items-center gap-1.5 text-xs text-white/40"><CalendarDays className="size-3.5" />{upcoming[0].scheduled_at ? formatDate(upcoming[0].scheduled_at, locale, true) : t('To be determined')}</p></div> : <p className="mt-3 text-sm text-white/45">{t('Schedule will be shown after publication.')}</p>}
                                </div>
                            </div>
                        </aside>
                    </div>
...
function ProgressMetric({ value, label, icon: Icon }: { value: number; label: string; icon: ComponentType<{ className?: string }> }) { return <div className="rounded-2xl border border-white/10 bg-white/5 p-4"><Icon className="size-4 text-[var(--public-highlight)]" /><b className="public-display mt-4 block text-3xl font-extrabold">{value}</b><span className="text-xs text-white/45">{label}</span></div>; }
function SectionHeading({ eyebrow, title, description }: { eyebrow: string; title: string; description: string | null }) { return <div className="max-w-2xl"><p className="text-[11px] font-black uppercase tracking-[.22em] text-[#23745f]">{eyebrow}</p><h2 className="mt-3 text-3xl font-black tracking-[-.04em] sm:text-5xl">{title}</h2>{description && <p className="mt-4 text-sm leading-6 text-[#687b74] sm:text-base">{description}</p>}</div>; }
...
function MatchCard({ match, locale, t, result }: { match: Match; locale: string; t: Translate; result: boolean }) { return <article className="rounded-2xl border border-[#10251f]/10 bg-[#f8faf6] p-4 transition hover:border-[#23745f]/40"><div className="flex flex-wrap items-center justify-between gap-2 text-[10px] font-black uppercase tracking-[.14em] text-[#23745f]"><span>{match.sport} · {match.event}</span><span>{match.scheduled_at ? formatDate(match.scheduled_at, locale, true) : t('To be determined')}</span></div><div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-center gap-3"><TeamLabel team={match.home} /><div className="rounded-xl bg-[#10251f] px-3 py-2 text-center text-xs font-black text-white">{result ? `${match.score_home ?? '–'} : ${match.score_away ?? '–'}` : 'VS'}</div><TeamLabel team={match.away} away /></div>{match.venue && <p className="mt-4 flex items-center gap-1.5 border-t border-[#10251f]/10 pt-3 text-xs text-[#687b74]"><MapPin className="size-3.5" />{match.venue}</p>}</article>; }
function TeamLabel({ team, away = false }: { team: Team; away?: boolean }) { return <div className={`flex min-w-0 items-center gap-2 ${away ? 'flex-row-reverse text-right' : ''}`}>{team?.logo_url && <img src={team.logo_url} alt="" className="size-9 shrink-0 object-contain" />}<b className="min-w-0 truncate text-sm">{team?.name || '–'}</b></div>; }
function MedalCard({ row, t }: { row: MedalRow; t: Translate }) { return <article className="flex items-center gap-4 rounded-2xl border border-[#10251f]/10 bg-white p-5"><span className="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-[#10251f] text-lg font-black text-[#d7ef59]">{row.rank}</span>{row.logo_url && <img src={row.logo_url} alt="" className="size-12 shrink-0 object-contain" />}<div className="min-w-0 flex-1"><h3 className="truncate font-black">{row.participant_name}</h3><p className="mt-1 text-xs text-[#687b74]">{row.gold} {t('Gold')} · {row.silver} {t('Silver')} · {row.bronze} {t('Bronze')}</p></div><b className="text-2xl">{row.total_medals}</b></article>; }
function EmptyState({ text }: { text: string }) { return <div className="rounded-2xl border border-dashed border-[#10251f]/20 bg-[#f8faf6] p-8 text-center text-sm text-[#687b74]">{text}</div>; }
