...
import { Activity, ArrowUpRight, CalendarDays, Clock3, LogIn, MapPin, Medal, RefreshCw, Trophy, Users } from 'lucide-react';
...
function MatchCard({ match, result = false, t }: { match: Match; result?: boolean; t: (key: string) => string }) {
    return <article className="group rounded-2xl border border-slate-200/90 bg-white p-5 shadow-[0_8px_30px_-20px_rgba(15,23,42,.35)] transition duration-200 hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-lg">
        <div className="flex items-center justify-between gap-3 text-xs font-semibold uppercase tracking-wider text-emerald-700"><span className="truncate">{match.sport || t('Event')} · {match.event}</span><span className={match.status === 'in_progress' ? 'rounded-full bg-red-50 px-2 py-1 text-red-600' : 'whitespace-nowrap text-slate-400'}>{match.status === 'in_progress' ? t('In progress') : result ? t('Completed') : formatDate(match.scheduled_at, true)}</span></div>
        ...
        <div className="min-h-screen bg-[#f6f8f7] text-slate-950">
            <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 text-slate-950 backdrop-blur">
                <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6"><a href="/portal/" className="flex items-center gap-3"><img src="/portal/images/utem-logo.svg" alt="UTeM" className="h-16 w-auto object-contain" /><span><small className="block text-xs tracking-[.2em] text-emerald-600">Portal Rasmi</small><b className="block text-base leading-tight">{app_name || 'SAF 2026'}</b></span></a>
                    <nav className="hidden gap-7 text-sm text-slate-600 md:flex"><a href="#pingat" className="hover:text-slate-950">{t('Medal standings')}</a><a href="#jadual" className="hover:text-slate-950">{t('Schedule')}</a><a href="#keputusan" className="hover:text-slate-950">{t('Results')}</a></nav><div className="flex items-center gap-2"><LocaleSwitcher compact showLabel={false} /><Link href={route('login')} className="flex items-center gap-2 rounded-full border border-slate-300 px-4 py-2 text-sm text-slate-700 hover:bg-slate-100"><LogIn className="size-4" /> {t('Log in')}</Link></div></div>
            </header>
            <main id="utama">
                ...
                <section aria-label="Ringkasan pertandingan" className="border-y border-slate-200/80 bg-white"><div className="mx-auto grid max-w-7xl grid-cols-2 divide-x divide-y divide-slate-200/80 sm:grid-cols-4 sm:divide-y-0">{[[stats.sports,'Sukan',Trophy],[stats.events,'Acara',Medal],[stats.faculties,'Kontinjen',Users],[stats.completed_matches,'Keputusan',Activity]].map(([value,label,Icon]: any)=><div key={label} className="p-5 sm:p-7"><Icon className="mb-5 size-5 text-emerald-600"/><b className="block text-3xl tracking-tight text-slate-950">{value}</b><span className="mt-1 block text-sm text-slate-500">{label}</span></div>)}</div></section>
                {sports.length > 0 && <section className="mx-auto max-w-7xl px-4 pb-2 pt-14 sm:px-6"><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-emerald-600">Program</p><h2 className="mt-2 text-2xl font-black tracking-tight">Sukan dipertandingkan</h2></div><Trophy className="hidden size-7 text-emerald-200 sm:block" /></div><div className="mt-5 flex flex-wrap gap-2">{sports.map(sport=><span key={sport} className="rounded-full border border-slate-200 bg-white px-3.5 py-2 text-sm font-medium text-slate-600 transition hover:border-emerald-300 hover:text-emerald-700">{sport}</span>)}</div></section>}
                <section id="jadual" className="mx-auto max-w-7xl px-4 py-16 sm:px-6"><div className="mb-7 flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-emerald-600">{t('Next')}</p><h2 className="mt-2 text-3xl font-black tracking-tight">{t('Match schedule')}</h2><p className="mt-2 max-w-xl text-sm text-slate-500">Ikuti perlawanan yang akan berlangsung dan rancang sokongan anda.</p></div><a href="#keputusan" className="hidden items-center gap-1 text-sm font-semibold text-emerald-700 sm:flex">Keputusan terkini <ArrowUpRight className="size-4" /></a></div>{upcoming.length ? <div className="grid gap-4 lg:grid-cols-2">{upcoming.map(match=><MatchCard key={match.id} match={match} t={t}/>)}</div> : <Empty text={t('Schedule will be shown after publication.')} />}</section>
                <section id="keputusan" className="border-t border-slate-200/80 bg-white py-16"><div className="mx-auto max-w-7xl px-4 sm:px-6"><div className="mb-7 flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-emerald-600">{t('Latest')}</p><h2 className="mb-1 mt-2 text-3xl font-black tracking-tight">{t('Match results')}</h2><p className="text-xs text-slate-500">{t('Updated')} {formatDate(updated_at, true)} · {t('Auto-refreshes every 30 seconds')}</p></div><button type="button" onClick={() => refresh()} disabled={refreshing} className="flex items-center gap-2 rounded-full border border-emerald-600/30 bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-100 disabled:opacity-60"><RefreshCw className={`size-4 ${refreshing ? 'animate-spin' : ''}`} />{refreshing ? t('Refreshing') : t('Refresh')}</button></div>{results.length ? <div className="grid gap-4 lg:grid-cols-2">{results.map(match=><MatchCard key={match.id} match={match} result t={t}/>)}</div> : <Empty text={t('No official results recorded yet.')} />}</div></section>
