        ...
        <PublicLayout title={competition?.name || app_name} appName={app_name} current="home" description={competition?.description || t('Official competition schedules, results, athletes and medal standings.')} canonical={route('public.index')}>
            ...
            <main>
                ...
                <section className="public-below-fold border-y border-[var(--public-dark-border)] bg-white py-20 sm:py-24"><div className="mx-auto max-w-7xl px-4 sm:px-6">
                    <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><PublicSectionHeading eyebrow={t('Latest')} title={t('Live competition updates')} description={`${t('Updated')} ${formatDate(updated_at, locale, true)}`} /><div className="mt-3"><PublicStaleDataNotice updatedAt={updated_at} /></div></div><div className="flex flex-col items-start gap-2 sm:items-end"><Button type="button" variant="outline" onClick={() => refresh(true)} disabled={refreshing} className="min-h-11 gap-2 self-start border-[var(--public-dark-border)] bg-white text-sm font-black hover:border-[var(--public-primary-border)] hover:text-[var(--public-primary)] sm:min-h-11 sm:self-auto"><RefreshCw className={`size-4 ${refreshing ? 'animate-spin' : ''}`} />{t(refreshing ? 'Refreshing' : 'Refresh')}</Button><p role="status" aria-live="polite" className={`min-h-5 text-xs font-semibold ${refreshStatus === 'error' ? 'text-red-700' : 'text-[var(--public-primary)]'}`}>{refreshStatus === 'success' ? t('Refresh complete') : refreshStatus === 'error' ? t('Unable to refresh. Please try again.') : ''}</p></div></div>
                ...
                <section className="public-below-fold border-y border-[var(--public-dark-border)] bg-[var(--public-background)] py-20 sm:py-24"><div className="mx-auto max-w-7xl px-4 sm:px-6">
                    ...
                    <div className="mt-12 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">{faculties.map(faculty => <Link key={faculty?.name} href={route('public.faculties')} aria-label={faculty?.name ?? t('Faculty')} className="public-card group flex min-h-36 flex-col items-center justify-center p-4 text-center"><span className="relative z-10"><ParticipantLogo participant={faculty} size="xl" alt="" className="size-20" /></span><span className="relative z-10 mt-3 line-clamp-2 text-xs font-black text-[var(--public-text)]">{faculty?.name ?? t('Faculty')}</span></Link>)}</div>
                </div></section>
