...
class SAF2026DataSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Session (SAF Ke-20 2026: 13-25 October 2026)
        $session = Session::withTrashed()->updateOrCreate(
            ['slug' => 'saf-2026', 'organization_id' => $orgId],
            [
                'organization_id' => $orgId,
                'name' => 'Sukan Antara Fakulti Ke-20 2026',
                'slug' => 'saf-2026',
                'description' => 'Sukan Antara Fakulti Ke-20 2026 - Universiti Teknikal Malaysia Melaka',
        ...
        // Single tournament spanning the full competition window.
        $tournament = Tournament::withTrashed()->updateOrCreate(
            ['slug' => 'saf-2026', 'session_id' => $session->id],
            [
                'organization_id' => $orgId,
                'session_id' => $session->id,
                'name' => 'Sukan Antara Fakulti Ke-20 2026',
                'slug' => 'saf-2026',
                'description' => 'Sukan Antara Fakulti Ke-20 2026 - 13-25 Oktober 2026',
                'start_date' => '2026-10-13',
                'end_date' => '2026-10-25',
        ...
        // Retire the legacy two-phase tournaments so re-seeding converges on one tournament.
        Tournament::query()
            ->where('session_id', $session->id)
            ->whereIn('slug', ['saf-2026-fasa-1', 'saf-2026-fasa-2'])
            ->delete();

        // Sports config (single tournament)
        $sportsConfig = [
            ['name' => 'Badminton', 'cats' => [
        ...
        $this->createHockeyPoolsAndFixtures($orgId, $tournament, $faculties);
    }
