...
## 2026-09-14 - Collection Aggregation Performance in Loops
**Learning:** Iterating over a parent collection (like `registrations`) and repeatedly calling collection filter methods (like `->where('role', ...)->count()`) on nested relations in a loop can be unexpectedly slow as the dataset grows, due to repeated iterations over the same nested collections.
**Action:** When aggregating counts across multiple nested collections, flatten them first (e.g., `$registrations->flatMap->squadMembers`) and use a single aggregation pass (e.g., `->countBy('role')`) to significantly reduce processing overhead.
