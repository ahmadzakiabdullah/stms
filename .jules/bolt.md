...
## 2024-08-04 - [Optimize Laravel Collection Aggregations]
**Learning:** Multiple `->where(...)->count()` calls on the same collection inside a loop lead to O(N*M) complexity.
**Action:** Replace multiple `->where(...)->count()` calls with a single `->countBy(...)` pass on the collection to evaluate the counts in O(M) time.
