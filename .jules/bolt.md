...
## 2026-08-01 - [Optimize Squad Member Role Aggregation]
**Learning:** Using multiple `->where(...)->count()` loops on the same collection inside of an outer `foreach` loop performs very poorly as the dataset scales. Profiling shows it takes a significant amount of time parsing through collections multiple times.
**Action:** Always replace multiple passes with a single `->countBy(...)` pass on collections to significantly reduce complexity and save processing time.
